...
class EventProducers(Document):
	...
	def on_update(self):
		if not self.incoming_change:
			if frappe.db.exists("Event Producers", self.name):
	...
	def set_last_update(self, last_update):
		...
		if not last_update_doc_name:
			...
			frappe.db.set_value(
				"Event Producers Last Update", last_update_doc_name, "last_update", last_update
			)
	...
	def update_event_consumer(self):
		if self.is_producer_online():
			...
			event_consumer = producer_site.get_doc("Event Consumers", get_url())
	...
	def is_producer_online(self):
		"""check connection status for the Event Producers site"""
		retry = 3
		while retry > 0:
...
def get_producer_site(producer_url):
	...
	producer_doc = frappe.get_doc("Event Producers", producer_url)
	producer_site = FrappeClient(
		url=producer_url,
...
def pull_producer_data():
	...
	if response.status_code == 200:
		for event_producers in frappe.get_all("Event Producers"):
			pull_from_node(event_producers.name)
		return "success"
	return None

...
class EventConsumers(Document):
	...
	def on_trash(self):
		for i in frappe.get_all("Event Update Log Consumers", {"consumer": self.name}):
			frappe.delete_doc("Event Update Log Consumers", i.name)

	def update_consumer_status(self):
		consumer_site = get_consumer_site(self.callback_url)
		event_producer = consumer_site.get_doc("Event Producers", get_url())
		...
		for entry in config:
			...
			entry["status"] = frappe.db.get_value(
				"Event Consumers Document Type", {"parent": self.name, "ref_doctype": ref_doctype}, "status"
			)
...
def get_consumer_site(consumer_url):
	...
	consumer_doc = frappe.get_doc("Event Consumers", consumer_url)
...
def get_last_update():
	...
	updates = frappe.get_list(
		"Event Update Logs", "creation", ignore_permissions=True, limit=1, order_by="creation desc"
	)
	if updates:
...
def notify_event_consumerss(doctype):
	...
	for entry in event_consumerss:
		consumer = frappe.get_doc("Event Consumers", entry.parent)
...
def notify(consumer):
	...
	if consumer_status == "online":
		try:
			...
			client.post_request(
				{
					"cmd": "event_streaming.event_streaming.doctype.event_producers.event_producers.new_event_notification",
	...
	if not consumer.flags.notified:
		enqueued_method = "event_streaming.event_streaming.doctype.event_consumers.event_consumers.notify"
		jobs = get_jobs()

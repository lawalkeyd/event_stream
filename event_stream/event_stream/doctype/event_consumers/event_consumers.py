...
def notify(consumer):
	...
	if consumer_status == "online" and consumer.is_producer_offline:
		...
		try:
			...
			client.post_request(
				{
					"cmd": "event_stream.event_stream.doctype.event_producers.event_producers.new_event_notification",
	...
	if not consumer.flags.notified:
		enqueued_method = "event_stream.event_stream.doctype.event_consumers.event_consumers.notify"
		jobs = get_jobs()

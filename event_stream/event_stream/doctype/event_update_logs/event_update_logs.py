...
class EventUpdateLogs(Document):
	def after_insert(self):
		"""Send update notification updates to event consumers
		whenever update log is generated"""
		enqueued_method = (
			"event_streaming.event_streaming.doctype.event_consumers.event_consumers.notify_event_consumers"
		)
		jobs = get_jobs()
...
def notify_consumers(doc, event):
	...
	if consumers:
		if event == "after_insert":
			doc.flags.event_update_logs = make_event_update_logs(doc, update_type="Create")
		elif event == "on_trash":
			make_event_update_logs(doc, update_type="Delete")
		else:
			# on_update
			# called after saving
			if not doc.flags.event_update_logs:  # if not already inserted
				diff = get_update(doc.get_doc_before_save(), doc)
				if diff:
					doc.diff = diff
					make_event_update_logs(doc, update_type="Update")
...
def make_event_update_logs(doc, update_type):
...
def is_consumer_uptodate(update_log, consumer):
	...
	prev_log_consumers = frappe.get_all(
		"Event Update Log Consumers",
		fields=["consumer"],
		filters={
			"parent": prev_logs[0].name,
			"parenttype": "Event Update Logs",
...
def mark_consumer_read(update_log_name, consumer_name):
	...
	"""
	update_log = frappe.get_doc("Event Update Logs", update_log_name)
	if len([x for x in update_log.consumers if x.consumer == consumer_name]):
		return

	frappe.get_doc(
		frappe._dict(
			doctype="Event Update Log Consumers",
...
def get_unread_update_logs(consumer_name, dt, dn):
	...
	already_consumed = [
		...
		SELECT
			update_log.name
		FROM `tabEvent Update Logs` update_log
		JOIN `tabEvent Update Log Consumers` consumer ON consumer.parent = %(log_name)s
		WHERE
...
def get_update_logs_for_consumer(event_consumer, doctypes, last_update):
	...
	from event_streaming.event_streaming.doctype.event_consumers.event_consumers import has_consumer_access
